...
//---------------------------------------------------INSTRUCTIONS:---------------------------------------------------
/*
    1)Our Graph is an array of Vertices where,
    2) Each vertex is an Edge d.s. linked-list with all containing alll the points(dest) adjactent to that corresponding
   Vertex (source)
*/
//----------------------------------------------------THE REAL DEAL----------------------------------------------------
class Graph
...
//-----------This edge type function clasifies all the edges according to tree edge, back edge  or forward edge----------
    void edge_type()
    {
    	int idx;
...
//-------------------------------------------------LoPoint CALCULATION----------------------------------------------------------------
//lowpoint is the lowest indexed vertex to which we can have a path to, from the current indexed vertex 'a'
...
//-----------------------------------------------PHI Function=------------------------------------------------------------------------
//this phi function gives us a phi value which is based on the low point of a vertex and this value is used in phi-sort
...
//-------------------------------------------------PHI SORT-----------------------------------------------------------------
//phi-sort will sort the edges into ascending order of their phi values
//in the end we will have a single topsorted -phi sorted graph which does not have redundant edges and is
//simplified form of the original graph
            ...
            do
                {
                    ...
                    while (ptr1->next != lptr)
                    {
                        ...
                        {
                            //swapping every atribute of that component
...
//-----------------------------------------------PATH FUNCTION------------------------------------------------------------------------
    ...
    /*
    the seq is a stack that stores the edges according to their traversal(like in dfs) and pops them when we no longer
    have unvisited edges from that vertex.
    the path function will basically decompose the graph , visually into all sets of   simple cycles or paths with
    mutually exclusive edges.This we will require during segmentation.

    NOTE THAT path function's outermost for loop does the job of traversing all the adjacent edges to a give vertex
    and it becomes clear that we are traversing all the vertices from the path function, from the recursive call,
    if we draw the recursion tree.

    ALSO recall from your concept that recursion is a top down process, but since its uses stack, the last entries will be
    computed first and then the result is forwards to upper part. so we get to check for the vertex 10's total_visit before
    checking vertex 1's total_visit condition
    */
    ...
    //---------------------------------PRINT THE PATHS (UTILITY FUNCTION)---------------------------------------------------
    void printPath()
    {
        int i;
